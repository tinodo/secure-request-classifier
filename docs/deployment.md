...
| Resource | Note |
| --- | --- |
| Deployment app registration | With **two federated identity credentials** (the `main` branch and the `demo` environment). No client secret is created, and there is deliberately no `pull_request` subject |
